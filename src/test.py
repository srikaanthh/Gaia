...
if __name__ == "__main__":
    ...
    model.eval()
    # cuDNN autotuner for fixed-size inputs can speed up inference
    ...
    try:
        for filename in img_list:
            ...
            try:
                ...
                mask_patches = np.empty((n_rows, n_cols, patch_size, patch_size), dtype=np.uint8)
            except Exception as e:
            ...
            # model prediction - OPTIMIZED with batch processing
            print(f"\nPredicting image file {filename}...")
            logger.info(f"Predicting image file {filename}...")
            try:
                # Flatten patches to a list: (n_rows*n_cols, H, W, C)
                flat_patches = patches.reshape(-1, patch_size, patch_size, 3)
                total_patches = flat_patches.shape[0]
                batch_size_inference = 16  # tune based on GPU memory

                flat_preds = np.empty((total_patches, patch_size, patch_size), dtype=np.uint8)

                with torch.no_grad():
                    for start in tqdm(range(0, total_patches, batch_size_inference), desc="Infer patches"):
                        end = min(start + batch_size_inference, total_patches)
                        batch = flat_patches[start:end]
                        # preprocess each image in batch
                        batch_proc = [preprocessing_fn(img).transpose(2, 0, 1).astype('float32') for img in batch]
                        x_tensor = torch.from_numpy(np.stack(batch_proc, axis=0)).to(device)
                        # forward
                        pred = model.predict(x_tensor)
                        # take argmax over channel dimension (classes)
                        pred_idx = pred.argmax(dim=1).cpu().numpy().astype(np.uint8)  # (B, H, W)
                        flat_preds[start:end] = pred_idx

                # reshape back to (n_rows, n_cols, H, W)
                mask_patches = flat_preds.reshape(n_rows, n_cols, patch_size, patch_size)
            except Exception as e:
