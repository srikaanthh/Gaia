...
if __name__ == "__main__":
    ...
    try:
        # cuDNN autotuner for fixed-size inputs can speed up training
        ...
        for i in range(0, epochs):
            ...
            if max_score < valid_logs['iou_score']:
                ...
                model_save_path = f'{model_dir}/landcover_{model_arch}_{encoder}_{optimizer_choice}_epochs{i}_patch{patch_size}_batch{batch_size}.pth'
                torch.save(model, model_save_path)
                logger.info(f'Best model saved with IoU score: {max_score:.4f}')
                print(f'Current best model saved with IoU score: {max_score:.4f}')
    ...
    # Optional: Clean up patches directory (can be disabled for reuse)
    # Uncomment if you want to delete patches after training
    # shutil.rmtree(patches_dir)
    
    print("\nPatches preserved in:", patches_dir)
    logger.info(f"Patches preserved in: {patches_dir} for potential reuse in future training")